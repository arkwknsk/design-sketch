...
export class AppManager {
  ...
  private _currentDegree: number[][] = Array.from({ length: Context.GRID_DIVISIONS_X }, () =>
  ...
  private _disks: Disk[][] = Array.from({ length: Context.GRID_DIVISIONS_X }, () => []);
  ...
  private initDebugPanel = (): void => {
    ...
    // noisePane.addMonitor(GameManager.PARAMS_NOISE, 'amplitude', {});
  ...
  update(): void {
    ...
    for (let i = 0; i < (Context.GRID_DIVISIONS_X - this._cOffset * 2 + 1); i++) {
      for (let j = 0; j < (Context.GRID_DIVISIONS_Y - this._rOffset * 2 + 1); j++) {
        this._currentFrequency += Math.floor((((this._currentWeatherData.wind_speed * 0.8) - this._currentFrequency) / 32) * 10) / 10;
        this._currentAmplitude += ((this._currentWeatherData.wind_speed * 0.1) - this._currentAmplitude) / 16;
        AppManager.PARAMS_NOISE.frequency = this._currentFrequency;
        AppManager.PARAMS_NOISE.amplitude = this._currentAmplitude;

        const px = j / (Context.GRID_DIVISIONS_X - this._cOffset * 2) * AppManager.PARAMS_NOISE.scale + time;
        const py = i / (Context.GRID_DIVISIONS_Y - this._rOffset * 2) * AppManager.PARAMS_NOISE.scale;

        const n = noise.perlin2(px * this._currentFrequency, py * this._currentFrequency) * this._currentAmplitude;
        this._currentDegree[i][j] = 360 * n + this._currentWeatherData.wind_direction_angle + 180;

        this._disks[i][j].angle += (this._currentDegree[i][j] - this._disks[i][j].angle) / 16;
        if (i === 0 && j === 0) AppManager.PARAMS_GENERAL.n = n;
  ...
  private setInitialDegree(): void {
    ...
    for (let i = 0; i < Context.GRID_DIVISIONS_X - 1 - cOffset * 2; i++) {
      for (let j = 0; j < Context.GRID_DIVISIONS_Y - 1 - rOffset * 2; j++) {
        this._currentDegree[i][j] = this._currentWeatherData.wind_direction_angle;
        this._disks[i][j].angle = this._currentDegree[i][j];
      }
    }
  }
